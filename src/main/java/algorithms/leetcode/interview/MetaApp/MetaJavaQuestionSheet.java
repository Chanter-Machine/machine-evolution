...
// 本试卷唯一 ID: 53D5DACE3FB55B14CED13E0E71F21ABA, 请勿修改本行内容
/* 注意, 这里不要写包名. 留空 */
...
import java.util.*;
/**
 * 回答方式: 直接保存或者复制本java文件, 然后在原处作答. 建议重命名成 javac -encoding UTF-8 xxxx.java
 * 选择题改变量赋值的字符串
...
class MetaJavaQuestionSheet {
    ...
    */
    ...
    public static boolean searchMatrix(int[][] nums, int x) {
        ...
        if(n < m){
            while(begin < n && nums[begin][m - 1] < x)
                begin++;
            while(begin < n && nums[begin][0] <= x){
                if(findTarget(nums,begin,1,x))
                    return true;
                begin++;
            }
            return false;
            ...
            while(begin < m && nums[n-1][begin] < x)
                begin++;
            while(begin < m && nums[0][begin] <= x){
                if(findTarget(nums,begin,0,x))
                    return true;
                begin++;
            }
        }
    ...
    public static boolean findTarget(int[][] matrix, int begin, int flag, int target){
        int low = 0;
        int high,mid;
    ...
    public static Set<String> showMap(Map<String, Object> map) {
        ...
        for (String key : keySet) {
            ...
            if (object == null) {
                ...
                resultSet.add(key + " = " + object);
            } else if (object instanceof String) {
                resultSet.add(key + " = " + object);
            } else if (object instanceof Map<?, ?>) {
                Set<String> showMap = showMap((Map<String, Object>) object);
                for (String subString : showMap) {
                    resultSet.add(key + "." + subString);
                }
            } else if (object instanceof Object[]) {
                Object[] objArray = (Object[]) object;
                Set<String> showArray = showArray(objArray);
                for (String subString : showArray) {
                    resultSet.add(key + subString);
                }
            }
        }
    ...
    public static Set<String> showArray(Object[] objArray) {
        Set<String> resultSet = new HashSet<String>();
        int i = 0;
        for (Object object : objArray) {
            if (object == null) {
                resultSet.add("[" + i + "] = " + "null");
            } else if (object instanceof Number) {
                resultSet.add("[" + i + "] = " + object);
            } else if (object instanceof String) {
                resultSet.add("[" + i + "] = " + object);
            } else if (object instanceof Map<?, ?>) {
                Set<String> showMap = showMap((Map<String, Object>) object);
                for (String subString : showMap) {
                    resultSet.add("[" + i + "]." + subString);
                }
            } else if (object instanceof Object[]) {
                Object[] objectArray = (Object[]) object;
                Set<String> showArray = showArray(objectArray);
                for (String subString : showArray) {
                    resultSet.add("[" + i + "]" + subString);
                }
            }
